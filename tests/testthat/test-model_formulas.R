...
test_that("IC subject-total", {
  # calculates sub_ttl correlation for each lambda
  expect_equal(length(eq$ic_sub_ttl(c(.5,.6))), 2)
  ...
  lams = c(.5,.5,.6)
  root_rel1 = sqrt(eq$reliability(c(.5,.6)))
  root_rel2 = sqrt(eq$reliability(c(.5, .5)))
  expect_equal(eq$ic_sub_ttl(lams), 
               c(lams[1:2]*root_rel1, lams[3]*root_rel2)
  )
})
...
test_that("BG total-total", {
  # depends on sample size
  expect_less_than(eq$bg_ttl_ttl(rep(.5, 5 ), rep(.5, 10), 1),
                   eq$bg_ttl_ttl(rep(.5, 10), rep(.5, 10), 1)
  )
  # depends on rho
  fives = rep(.5, 10)
